...
# Use exact same config as local - no changes
# Just fix the current issues with existing setup
...
print_status "Using exact same config as local - no changes..."
...
# Now build and start with exact same config as local
print_status "Building and starting with exact same config as local..."
