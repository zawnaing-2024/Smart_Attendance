...
# Wait for services to fully start and test them
# The services are still building/installing packages
...
print_status "Waiting for services to fully start and testing them..."
...
print_status "Waiting longer for services to be ready..."
sleep 60
...
# Check if face_detection service exists
print_status "Checking if face_detection service exists..."
docker ps -a | grep face_detection || print_warning "Face detection container not found"

# If face_detection service doesn't exist, start it
if ! docker ps -a | grep face_detection; then
    print_status "Starting face_detection service..."
