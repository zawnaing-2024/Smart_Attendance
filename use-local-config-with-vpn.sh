...
# Use exact same config as local with VPN bypass
# Clean slate approach with original working configuration
...
print_status "Using exact same config as local with VPN bypass..."
...
print_status "Starting VPN and building with exact same config as local..."

# Wait for VPN to be ready
print_status "Waiting for VPN to be ready..."
sleep 10
...
if ping -c 1 8.8.8.8 > /dev/null 2>&1; then
    ...
    print_warning "Internet connectivity issues, but continuing..."
fi
...
if curl -f http://deb.debian.org/debian/ > /dev/null 2>&1; then
    ...
    print_warning "Debian repository not accessible, but continuing..."
fi
...
print_success "Exact same config deployment completed!"
